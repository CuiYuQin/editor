  ...
  <div :class="[{ fullscreen: s_fullScreen, shadow: boxShadow }]" class="v-note-wrapper markdown-body"
    ...
    <div class="v-note-op" v-show="toolbarsFlag" :style="{ background: toolbarsBackground }">
      ...
      <v-md-toolbar-right ref="toolbar_right" :d_words="d_words" @toolbar_right_click="toolbar_right_click"
        :toolbars="toolbars" :s_subfield="s_subfield" :s_preview_switch="s_preview_switch" :s_fullScreen="s_fullScreen"
        :s_html_code="s_html_code" :s_navigation="s_navigation" :class="{ transition: transition }">
    ...
    <div class="v-note-panel">
      <!--编辑区-->
      <div ref="vNoteEdit" @scroll="$v_edit_scroll" class="v-note-edit divarea-wrapper" :class="{
        ...
        <div class="content-input-wrapper" :style="{ 'background-color': editorBackground }">
          ...
          <v-autoTextarea ref="vNoteTextarea" :placeholder="placeholder ? placeholder : d_words.start_editor"
            class="content-input" :fontSize="fontSize" lineHeight="1.5" v-model="d_value" fullHeight
            :style="{ 'background-color': editorBackground }" :backgroundColor="'#363B40'" :color="'#B8BFC6'">
          </v-autoTextarea>
        </div>
      </div>
      <!--展示区-->
      <div :class="{
        ...
        <div ref="vShowContent" v-html="d_render" v-show="!s_html_code" :class="{
            'scroll-style': s_scrollStyle,
            'scroll-style-border-radius': s_scrollStyle,
          }" class="v-show-content" :style="{ 'background-color': previewBackground }"></div>
        <div v-show="s_html_code" :class="{
            'scroll-style': s_scrollStyle,
            'scroll-style-border-radius': s_scrollStyle,
          }" class="v-show-content-html" :style="{ 'background-color': previewBackground }">
          {{ d_render }}
        </div>
      </div>
      ...
      <transition name="slideTop">
        <div v-show="s_navigation" class="v-note-navigation-wrapper" :class="{ transition: transition }">
          <div class="v-note-navigation-title">
            ...
            }}<i @click="toolbar_right_click('navigation')" class="fa fa-mavon-times v-note-navigation-close"
              aria-hidden="true"></i>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      $toolbar_right_read_change_status() {
        ...
        console.log(" if (this.readmodel) {", this.readmodel)
        if (this.readmodel) {
